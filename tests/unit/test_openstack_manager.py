...
from github_runner_image_builder import openstack_manager
from github_runner_image_builder.openstack_manager import (
    GetImageError,
    Image,
...
def openstack_manager_mock_fixture(monkeypatch: pytest.MonkeyPatch, connection: MagicMock):
    ...
    return openstack_manager.OpenstackManager(cloud_name=MagicMock())
...
def test_openstack_manager_context(monkeypatch: pytest.MonkeyPatch, connection: MagicMock):
    ...
    with openstack_manager.OpenstackManager(cloud_name=MagicMock()):
        pass
...
def test___init__error(monkeypatch: pytest.MonkeyPatch):
    ...
    with pytest.raises(UnauthorizedError) as exc:
        openstack_manager.OpenstackManager(cloud_name=MagicMock())
...
def test_get_latest_image_id_error(manager: OpenstackManager):
    ...
    with pytest.raises(GetImageError) as exc:
        manager.get_latest_image_id(name=MagicMock())
...
def test_get_latest_image_id(
    ...
    assert manager.get_latest_image_id(name=MagicMock()) == expected_id
